...
class AllContacts(InfoInterface):
    def print_info():
        ...
        ls_result = result.split("\n")    
        for row in ls_result:
            if row != "":
                row = row.replace("ObjectId(","")
                row = row.replace(")","")
                row_dict = json.loads(row.replace("'",'"'))
                list_info.append(row_dict.get('name',""))
                list_info.append(row_dict.get('address',""))
                list_info.append(row_dict.get('phone',""))
                list_info.append(row_dict.get('email',""))
                list_info.append(row_dict.get('birthday',""))
...
def add_contact():
    ...
    if AB.validate_birthday(birthday) and AB.validate_phone(phone):
        ...
        result = my_insert_one({"name": name, "phone": phone, "birthday": birthday})
...
def add_email():
    name = input('Enter Name: ')
    result_find = my_find_one({"name" : name})
    if result_find != b'None':
        email = input('Enter Email: ')
        if AB.validate_email(email):
            result = my_update_one({"name" : name}, \
...
def find_contact():
    ...
    if result != "":
        print(result)
        row = result.replace("ObjectId(","").replace(")","").replace("\n","")
        row_dict = json.loads(row.replace("'",'"'))
        list_info.append(row_dict.get('name',""))
        list_info.append(row_dict.get('address',""))
        list_info.append(row_dict.get('phone',""))
...
def delete_contact():
    ...
    result_find = my_find_one({"name" : name})
    if result_find != b'None':
        result = my_delete_one({"name": name})
        return f"This {name} contact is delete"
    else:
        return f"This {name} contact not found"
